...
def classify_with_bert(log_message):
    embeddings = model_embedding.encode([log_message]) # create embeddings
    
    # calculating probabilities and getting the first element (first element will be sorted as highest probability), this is done to detect unclassified case.
    probabilities = model_classification.predict_proba(embeddings)[0] 
    if max(probabilities) < 0.5:
        return "Unclassified"
    # predicted and returning label (for normal cases)
